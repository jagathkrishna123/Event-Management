...
import { IoLocationOutline, IoPeopleOutline, IoSnowOutline, IoTimeOutline, IoRestaurantOutline, IoCameraOutline, IoFlowerOutline, IoStarOutline, IoSearchOutline, IoFilterOutline } from "react-icons/io5";
...
const EventShowcasebyCategory = () => {
  ...
  const filteredServices = useMemo(() => {
    let services = allServices.filter((service) => service.category === category);
    ...
    if (sortBy === "price-low") {
      services.sort((a, b) => {
        ...
        if (category === "auditorium") {
          ...
          priceA = Math.min(...(a.packages?.map(p => p.pricePerPerson) || [0]));
          priceB = Math.min(...(b.packages?.map(p => p.pricePerPerson) || [0]));
  ...
  const getCategoryInfo = (category) => {
    switch(category) {
      case 'auditorium':
        return { name: 'Auditorium', icon: IoPeopleOutline, color: 'from-blue-500 to-blue-600' };
      case 'catering':
        return { name: 'Catering', icon: IoRestaurantOutline, color: 'from-green-500 to-green-600' };
      case 'photography':
        return { name: 'Photography', icon: IoCameraOutline, color: 'from-purple-500 to-purple-600' };
      case 'stage-decoration':
        return { name: 'Stage Decoration', icon: IoFlowerOutline, color: 'from-pink-500 to-pink-600' };
      default:
        return { name: category.replace('-', ' '), icon: IoStarOutline, color: 'from-gray-500 to-gray-600' };
    }
  };
  ...
  const renderServiceCard = (item) => {
      ...
      return (
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2 text-sm text-green-600 font-semibold">
            <span>₹{item.pricePerDay?.toLocaleString()}/day</span>
          </div>
        </div>
      );
    }
            ...
            <div key={pkg.id} className="bg-green-50 p-3 rounded-lg">
              <div className="flex justify-between items-start">
                ...
                <div className="text-right">
                  <span className="text-green-600 font-bold">₹{pkg.pricePerPerson}</span>
          ...
          {item.packages?.slice(0, 2).map((pkg, index) => (
            <div key={pkg.id} className="bg-purple-50 p-3 rounded-lg">
              <div className="flex justify-between items-start">
                ...
                <div className="text-right">
                  <span className="text-purple-600 font-bold">₹{pkg.pricePerHour}</span>
    ...
    if (category === 'stage-decoration') {
      return (
        <div className="space-y-3">
          {item.packages?.slice(0, 2).map((pkg, index) => (
            <div key={pkg.id} className={`p-3 rounded-lg ${pkg.category === 'Luxury' ? 'bg-yellow-50' : pkg.category === 'Premium' ? 'bg-blue-50' : 'bg-gray-50'}`}>
              <div className="flex justify-between items-start">
                <div>
                ...
                <div className="text-right">
                  <span className="text-green-600 font-bold">₹{pkg.pricePerDay?.toLocaleString()}</span>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-10 pt-20">
      <div className="max-w-7xl mx-auto px-4">
        {/* Header Section */}
        <div className="text-center mb-12">
          <div className={`inline-flex items-center gap-3 bg-gradient-to-r ${categoryInfo.color} text-white px-6 py-3 rounded-full mb-4`}>
          ...
          <p className="text-gray-600 max-w-2xl mx-auto">
            Discover the best {categoryInfo.name.toLowerCase()} services in your area. Compare prices, features, and book instantly.
          </p>
        </div>
            ...
            {filteredServices.map((item) => (
              ...
              >
                ...
                <div className="relative overflow-hidden">
                  <img
                    src={item.images?.[0]}
                    alt={item.auditoriumName || item.companyName || item.studioName}
                    className="h-48 w-full object-cover group-hover:scale-105 transition-transform duration-300"
                  />
                  <div className={`absolute top-4 right-4 bg-gradient-to-r ${categoryInfo.color} text-white px-3 py-1 rounded-full text-xs font-semibold`}>
                    {categoryInfo.name}
                  </div>
                </div>
